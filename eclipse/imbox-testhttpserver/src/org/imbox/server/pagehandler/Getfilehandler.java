...
import org.imbox.server.functions.Serverblockreader;
			...
			try
			{
				...
				}else
				{
					if (httpconnection.getRequestMethod().equals("POST"))
					{
						...
						{
							//token is correct
							Serverblockreader br = new Serverblockreader();
							String path = "/storage/" + reader.getfilename() +"/" +reader.getblockname();
							String data = br.getbyteofblockbypath(path);
							//return result
							if (data.length() >0)
							{
